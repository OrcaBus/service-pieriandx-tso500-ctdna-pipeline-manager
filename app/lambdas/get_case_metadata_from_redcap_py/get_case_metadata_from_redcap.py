...
def launch_redcap_label_lambda(library_id: str) -> pd.DataFrame:
    ...
    redcap_label_df.rename(
        columns={
            "report_type": "sampleType",
            "patient_gender": "gender",
            "disease": "diseaseName",
            "libraryid": "libraryId",
            "pierian_metadata_complete": "pierianMetadataComplete"
        },
        inplace=True
    )
...
def get_and_merge_raw_and_label_data(library_id: str) -> Dict:
    ...
    for date_column in ["dateCollected", "dateReceived", "timeCollected"]:
    ...
    for date_column in ["timeCollected"]:
...
def handler(event, context) -> Dict:
    ...
    try:
        return {
            "redcapData": get_and_merge_raw_and_label_data(library_id),
            "inRedcap": True
        }
    except ValueError:
        return {
