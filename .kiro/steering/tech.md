...
- **AWS Lambda** (Python, ARM64, 512 MB default / 1024 MB with PierianDx layer or pandas, 60s timeout; some lambdas use extended timeout)
- **AWS Step Functions** (ASL JSON templates in `app/step-functions-templates/`)
- **Amazon EventBridge** — event bus `OrcaBusMain`, source `orcabus.pieriandxtso500ctdna`
- **AWS SSM Parameter Store** — configuration under `/orcabus/workflows/pieriandx-tso500-ctdna/`
- **AWS Schemas Registry** — JSON schema validation for draft events
- **AWS Secrets Manager** — PierianDx S3 credentials and auth tokens
- **Amazon S3** — PierianDx lookup bucket (SNOMED maps) and cross-account transfer bucket
